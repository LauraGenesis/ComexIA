...
import { PackingPreview } from "./packing-preview";
import {
  PACKING_CAMPOS,
  PACKING_SECCIONES,
  lineaVacia,
  ...
  type PackingDatos,
  type PackingLinea,
} from "@/lib/documentos/packing";
import type { OrigenDocumento } from "@/lib/documentos/historial";

// Clave temporal donde el extractor deja el packing precargado (sin base de datos).
const PREFILL_KEY = "comexia:packing-prefill";

// Columnas editables de la tabla de bultos.
const COLUMNAS: { key: keyof PackingLinea; label: string; ancho: string }[] = [
  { key: "marcas", label: "Marcas y nº", ancho: "w-[14%]" },
  { key: "numBultos", label: "Bultos", ancho: "w-[7%]" },
  { key: "tipoBulto", label: "Tipo", ancho: "w-[9%]" },
  { key: "descripcion", label: "Descripción", ancho: "w-[22%]" },
  { key: "cantidad", label: "Cantidad", ancho: "w-[9%]" },
  { key: "unidad", label: "Unidad", ancho: "w-[8%]" },
  { key: "pesoNeto", label: "P. neto", ancho: "w-[8%]" },
  { key: "pesoBruto", label: "P. bruto", ancho: "w-[8%]" },
  { key: "dimensiones", label: "Dimens.", ancho: "w-[9%]" },
];
...
export function PackingEditor({
  ...
  inicial: PackingDatos;
  prefill?: boolean;
  origen?: OrigenDocumento;
  historialId?: string;
}) {
  const [d, setD] = useState<PackingDatos>(inicial);
  ...
  // Al llegar desde la extracción con IA, aplicamos el packing que el extractor
  // dejó en sessionStorage y lo consumimos (una sola vez) para no re-aplicarlo.
  useEffect(() => {
    if (!prefill) return;
    try {
      const raw = sessionStorage.getItem(PREFILL_KEY);
      if (raw) {
        const datos = JSON.parse(raw) as Partial<PackingDatos>;
  ...
  const problemas = useMemo(() => validarPacking(d), [d]);
  const errores = problemas.filter((p) => p.nivel === "error");
  const avisos = problemas.filter((p) => p.nivel === "aviso");

  function set<K extends keyof PackingDatos>(key: K, valor: PackingDatos[K]) {
    setD((prev) => ({ ...prev, [key]: valor }));
    setGuardado(false);
  }

  function setLinea<K extends keyof PackingLinea>(
    idx: number,
    key: K,
    valor: PackingLinea[K],
  ) {
    setD((prev) => ({
      ...prev,
  ...
  function anadirLinea() {
    setD((prev) => ({ ...prev, lineas: [...prev.lineas, lineaVacia()] }));
    setGuardado(false);
  }
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="no-print space-y-5">
        {PACKING_SECCIONES.filter((s) => s !== "Pie" && s !== "Totales").map(
          (seccion) => (
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            <CardTitle>Bultos y mercancía</CardTitle>
            <button
              type="button"
              onClick={anadirLinea}
          ...
          <CardBody className="space-y-3">
            {d.lineas.map((linea, i) => (
              <div
                key={i}
                className="relative rounded-lg border border-line bg-canvas/50 p-3"
              >
...
/** Grupo de campos planos (cabecera, partes, envío, totales, pie). */
function SeccionCampos({
  ...
  seccion: string;
  d: PackingDatos;
  set: <K extends keyof PackingDatos>(key: K, valor: PackingDatos[K]) => void;
}) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardBody className="grid gap-3 sm:grid-cols-2">
        {PACKING_CAMPOS.filter((c) => c.seccion === seccion).map((campo) => {
