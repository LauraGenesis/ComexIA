...
import { DgdPreview } from "./dgd-preview";
import {
  DGD_CAMPOS,
  DGD_SECCIONES,
  dgdLineaVacia,
...
const PREFILL_KEY = "comexia:dgd-prefill";

// La tabla de mercancías se inserta tras esta sección.
const SECCION_ANTES_TABLA = "Transporte";

// Columnas editables de la tabla de mercancías peligrosas.
// `nucleo` marca los datos ADR/IMDG que la validación exige por línea.
const COLUMNAS: {
  key: keyof DgdLinea;
  label: string;
  ancho?: string;
  ...
  { key: "unNumber", label: "Nº ONU", nucleo: true, placeholder: "UN1263" },
  { key: "clase", label: "Clase / división", nucleo: true, placeholder: "3" },
  { key: "grupoEmbalaje", label: "Grupo de embalaje", placeholder: "II" },
  { key: "designacionOficial", label: "Designación oficial de transporte", ancho: "sm:col-span-3", nucleo: true, placeholder: "PINTURA" },
  { key: "riesgoSecundario", label: "Riesgo secundario" },
  { key: "numBultos", label: "Nº y tipo de bultos", placeholder: "20 bidones" },
  { key: "cantidad", label: "Cantidad / masa neta" },
  { key: "pesoBruto", label: "Masa bruta (kg)" },
  { key: "puntoInflamacion", label: "Punto de inflamación (°C)", placeholder: "23" },
  { key: "contaminanteMarino", label: "Contaminante marino", placeholder: "Sí / —" },
  { key: "ems", label: "EmS", placeholder: "F-E S-E" },
];

export function DgdEditor({
  inicial,
  prefill = false,
  origen = "manual",
  historialId,
  ...
  const [d, setD] = useState<DgdDatos>(inicial);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const datos = JSON.parse(raw) as Partial<DgdDatos>;
  ...
  const problemas = useMemo(() => validarDgd(d), [d]);
  const errores = problemas.filter((p) => p.nivel === "error");
  const avisos = problemas.filter((p) => p.nivel === "aviso");

  function set<K extends keyof DgdDatos>(key: K, valor: DgdDatos[K]) {
    setD((prev) => ({ ...prev, [key]: valor }));
    setGuardado(false);
  }

  function setLinea<K extends keyof DgdLinea>(
    idx: number,
    key: K,
    valor: DgdLinea[K],
  ) {
  ...
  function anadirLinea() {
    setD((prev) => ({ ...prev, lineas: [...prev.lineas, dgdLineaVacia()] }));
    setGuardado(false);
  }
  ...
  const seccionesAntes = DGD_SECCIONES.slice(
    0,
    DGD_SECCIONES.indexOf(SECCION_ANTES_TABLA) + 1,
  );
  const seccionesDespues = DGD_SECCIONES.slice(
    DGD_SECCIONES.indexOf(SECCION_ANTES_TABLA) + 1,
  );
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="no-print space-y-5">
        <div className="flex items-start gap-2 rounded-lg border border-warning/40 bg-warning/5 px-3 py-2 text-sm text-ink">
          ...
          <p>
            La firma la aporta el <strong>expedidor</strong>, que declara y
            responde de la correcta clasificación, embalaje y etiquetado de la
            mercancía peligrosa. Verifica el Nº ONU, la clase y el grupo de
            embalaje con la ficha de datos de seguridad y el Código IMDG/ADR.
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Mercancías peligrosas</CardTitle>
            <button
              type="button"
              onClick={anadirLinea}
                    ...
                    return (
                      <div key={col.key} className={col.ancho}>
                        ...
                        <input
                          ...
                          className={`h-8 w-full rounded-md border bg-canvas px-2 text-sm focus:outline-none ${
                            falta
...
function SeccionCampos({
  ...
  seccion: string;
  d: DgdDatos;
  set: <K extends keyof DgdDatos>(key: K, valor: DgdDatos[K]) => void;
}) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardBody className="grid gap-3 sm:grid-cols-2">
        {DGD_CAMPOS.filter((c) => c.seccion === seccion).map((campo) => {
