...
import { FitosanitarioPreview } from "./fitosanitario-preview";
import {
  FITO_CAMPOS,
  FITO_SECCIONES,
  fitoLineaVacia,
...
// Clave temporal donde el extractor deja el borrador precargado (sin base de datos).
const PREFILL_KEY = "comexia:fitosanitario-prefill";

// La tabla de mercancías se inserta tras esta sección (casilla 4).
const SECCION_ANTES_TABLA = "Descripción del envío (casilla 4)";

// Columnas editables de la tabla de descripción del envío.
const COLUMNAS: { key: keyof FitoLinea; label: string }[] = [
  { key: "marcasBultos", label: "Marcas; nº y clase de bultos" },
  { key: "nombreProducto", label: "Nombre del producto" },
  { key: "nombreBotanico", label: "Nombre botánico (género y especie)" },
  { key: "cantidad", label: "Cantidad declarada" },
];

export function FitosanitarioEditor({
  inicial,
  prefill = false,
  origen = "manual",
  historialId,
  ...
  const [d, setD] = useState<FitoDatos>(inicial);
  ...
  // Al llegar desde la extracción con IA, aplicamos el borrador que el extractor
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const datos = JSON.parse(raw) as Partial<FitoDatos>;
  ...
  const problemas = useMemo(() => validarFito(d), [d]);
  const errores = problemas.filter((p) => p.nivel === "error");
  const avisos = problemas.filter((p) => p.nivel === "aviso");

  function set<K extends keyof FitoDatos>(key: K, valor: FitoDatos[K]) {
    setD((prev) => ({ ...prev, [key]: valor }));
    setGuardado(false);
  }

  function setLinea<K extends keyof FitoLinea>(
    idx: number,
    key: K,
    valor: FitoLinea[K],
  ) {
  ...
  function anadirLinea() {
    setD((prev) => ({ ...prev, lineas: [...prev.lineas, fitoLineaVacia()] }));
    setGuardado(false);
  }
  ...
  const seccionesAntes = FITO_SECCIONES.slice(
    0,
    FITO_SECCIONES.indexOf(SECCION_ANTES_TABLA) + 1,
  );
  const seccionesDespues = FITO_SECCIONES.slice(
    FITO_SECCIONES.indexOf(SECCION_ANTES_TABLA) + 1,
  );
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="no-print space-y-5">
        <div className="flex items-start gap-2 rounded-lg border border-warning/40 bg-warning/5 px-3 py-2 text-sm text-ink">
          ...
          <p>
            Este documento lo emite la autoridad oficial (Sanidad Vegetal / ONPF)
            tras inspección. Aquí preparas un <strong>borrador</strong> para
            agilizar la solicitud; no es válido sin la emisión oficial.
          </p>
        </div>
        ...
        {/* Tabla de mercancías (descripción del envío) */}
        <Card>
          <CardHeader>
            <CardTitle>Mercancías del envío</CardTitle>
            <button
              type="button"
              onClick={anadirLinea}
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {COLUMNAS.map((col) => (
                    <div
                      key={col.key}
                      className={
                        col.key === "marcasBultos" ? "sm:col-span-2" : ""
                      }
                    >
                      ...
                      <input
                        ...
                        className={`h-8 w-full rounded-md border bg-canvas px-2 text-sm focus:outline-none ${
                          col.key === "nombreBotanico" &&
                          (linea.nombreProducto.trim() ||
                            linea.marcasBultos.trim()) &&
                          !linea.nombreBotanico.trim()
                            ? "border-warning/60 focus:border-warning"
                            : "border-line focus:border-brand-500"
                        } ${col.key === "nombreBotanico" ? "italic" : ""}`}
                        placeholder={
                          col.key === "nombreBotanico"
                            ? "p. ej. Olea europaea"
                            : undefined
...
/** Grupo de campos planos del certificado fitosanitario. */
function SeccionCampos({
  ...
  seccion: string;
  d: FitoDatos;
  set: <K extends keyof FitoDatos>(key: K, valor: FitoDatos[K]) => void;
}) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardBody className="grid gap-3 sm:grid-cols-2">
        {FITO_CAMPOS.filter((c) => c.seccion === seccion).map((campo) => {
